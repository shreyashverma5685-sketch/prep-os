...
@app.get("/stats/weakness-5factor")
def get_five_factor_weakness():
    conn = get_connection()
    cursor = conn.cursor()
    ...
    for r in rows:
        ...
        mistake_cnt = r[3]
        avg_time = r[4] or 0.0
        avg_attempts = r[5] or 1.0
        avg_hints = r[6] or 0.0
        last_logged_str = r[7]

        days_since = 0
        if last_logged_str:
            try:
                ld = datetime.strptime(str(last_logged_str).split('T')[0].split(' ')[0], '%Y-%m-%d').date()
